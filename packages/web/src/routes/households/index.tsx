...
function HouseholdsPage() {
  ...
  function handleConfirm(s: HouseholdSuggestion) {
    const key = s.memberIds.join(',');
    setConfirmed(prev => new Set(prev).add(key));
  }

  function handleReject(s: HouseholdSuggestion) {
    const key = s.memberIds.join(',');
          ...
          <Card key={i}>
            ...
            <CardContent>
              ...
              <div className="flex gap-3">
                <Button onClick={() => handleConfirm(s)}>Confirm Group</Button>
                <Button variant="ghost" onClick={() => handleReject(s)}>Skip</Button>
              </div>
            </CardContent>
          </Card>
        ))}
