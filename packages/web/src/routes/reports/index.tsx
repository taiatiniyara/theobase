        ...
        <div className="mx-auto max-w-3xl">
          <Card>
            <CardContent className="py-12 text-center">
              ...
              <p className="mt-2 text-sm text-neutral-500">Could not load statistical report data. Please try again.</p>
        ...
        <div className="mx-auto max-w-lg">
          <Card>
            <CardContent className="py-12 text-center">
              ...
              <h2 className="mt-4 text-lg font-semibold text-neutral-900">Report Submitted</h2>
              <p className="mt-2 text-sm text-neutral-500">The {selectedYear} statistical report has been submitted to the Conference Secretary for review.</p>
        ...
        {report ? (
          <>
            <Card>
              <CardContent className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-6">
                <div><div className="text-sm text-neutral-500">Total Members</div><div className="text-2xl font-bold text-neutral-900">{report.totalMembers}</div></div>
                {(['q1','q2','q3','q4'] as const).map(q => (
                  <div key={q}>
                    <div className="text-sm text-neutral-500">{q.toUpperCase()}</div>
                    <div className="text-2xl font-bold text-neutral-900">{Object.values(report.quarters[q]).reduce((a, b) => a + b, 0)}</div>
                  </div>
                ))}
              </CardContent>
            </Card>
              ...
              <Card key={q}>
                ...
                <CardContent>
                  <dl className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    <div><dt className="text-sm text-neutral-500">Baptisms</dt><dd className="text-lg font-bold tabular-nums text-neutral-900">{report.quarters[q].baptised}</dd></div>
                    <div><dt className="text-sm text-neutral-500">Professions</dt><dd className="text-lg font-bold tabular-nums text-neutral-900">{report.quarters[q].profession}</dd></div>
          ...
          <Card>
            <CardContent className="py-12 text-center">
              ...
              <h2 className="mt-4 text-lg font-semibold text-neutral-900">No Report Data</h2>
              <p className="mt-2 text-sm text-neutral-500">No statistical data available for {selectedYear}. Try selecting a different year.</p>
            </CardContent>
          </Card>
        )}
      </div>
