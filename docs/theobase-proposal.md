...
Theobase is a platform for local Seventh-day Adventist churches to run their daily operations — membership records, finances, board minutes, ministry reporting, safety compliance, and communication — in a single tool that works on a phone or tablet, online and offline.

This proposal invites your conference or mission to advise on the platform's development and, when it is ready, to pilot it in one or two churches at no cost.
...
**Offline-first.** The platform runs fully on a local device. An officer in a remote area with no internet can record a baptism or count an offering. When a connection becomes available, data syncs. No officer ever waits on a network.
...
**Reporting as a byproduct.** Because officers use Theobase to perform their actual daily work, the quarterly statistical report and the remittance to the conference are not separate tasks. They are the natural output of work already done.

**Multi-lingual.** The interface can be presented in any language your field requires — not just with translated labels, but with workflows and role names adapted to each congregation's cultural context. A low-literacy mode uses icon-heavy interfaces and audio prompts for officers who find text-dense screens difficult.
...
| **Division-specific platforms** | Where a division or union has deployed platforms for pastoral reporting, member engagement, or ministry tracking, Theobase is designed to coexist — importing historical data and running in parallel during transition if desired. |
| **Adsafe / ALC** (training) | Theobase links to accredited training providers to verify course completion. It does not re-host their content. |
...
Before building anything specific to your field, we want to understand how things actually work on the ground:
...
- What connectivity constraints do churches face?

This phase requires one or two conversations — with conference staff who oversee local church administration, and ideally with a clerk or treasurer who does the work daily. No commitment beyond that.
...
- One or two churches, selected by the conference, receive free access.
...
- At the end of the pilot, the conference decides whether to continue, expand, or stop.

**Pilot scope:** The conference selects which churches participate. Ideal mix: one with reliable connectivity and one in an area where internet is unreliable, to test both online and offline operation.

---

## 6. What the Pilot Would Involve — Per Church
...
**Payment (post-pilot):**
- **Conference-covered:** The conference pays for its churches. Monthly invoice: number of churches × $3. Payment method to be agreed.
- **Church-direct:** Individual churches subscribe directly if the conference does not opt into bulk coverage.
...
**Data ownership.** The local church owns its data. The conference has access to aggregated reporting data for its churches. Theobase is a custodian, not an owner.

**Data residency.** Church data is stored in a region that complies with applicable data-protection requirements, configurable per field.
...
| Risk | Mitigation |
|---|---|
| Officers are unfamiliar with smartphones. | Low-literacy mode; interactive tutorials; practice mode with dummy data before going live. |
| Internet is unreliable. | Offline-first architecture. Platform works fully without a connection. Syncs when one is available. |
...
| The platform does not fit local workflows. | Phase 1 listening shapes the platform before the pilot. Regular check-ins catch misfits early during the pilot. |
| This is a new, unproven platform. | Honesty about development status. The ask is modest — advice today, a small pilot tomorrow. No large-scale commitment required. |
...
|---|---|
| Initial conversation with conference leadership | At the conference's convenience |
...
| Phase 2: Pilot churches onboarded | At the conference's discretion, once platform is ready |
| Pilot review | After 3–6 months of use |

All timelines are flexible and subject to the conference's priorities and the platform's development progress.

---

## 11. What We Are Asking

1. **A conversation** with someone who understands how local church administration works in your field — workflows, pain points, languages.
...
3. **Permission to pilot**, when the platform is ready, in one or two churches selected by the conference.

4. **Honest feedback** on what does and does not work.

There is no financial commitment, no timeline pressure, and no obligation to continue beyond the pilot.

---

## 12. What the Conference Gets

1. **A platform shaped by its context**, not a generic tool retrofitted after the fact.

2. **Cleaner, faster reporting** from pilot churches — quarterly reports and remittances generated automatically from daily work.

3. **Influence over the product roadmap.** Feedback from your officers during development means their needs are built in, not added later.

4. **First access.** Pilot churches use the platform before general availability.

5. **No cost.** The pilot is free. If the conference chooses to continue, the price is $3/church/month.

---

## 13. About the Platform

Theobase is built by Taia Tiniyara, a software engineering studio. It is not a division, union, or General Conference project. It is built on Cloudflare's infrastructure (Workers, D1, R2) and uses modern web standards (React, TypeScript). The codebase is designed to be maintainable with low operational overhead, which is what makes the $3/church price sustainable.

[taiatiniyara.com](https://taiatiniyara.com)

---

*This proposal is a starting point for conversation. All details are open to adjustment based on your needs, constraints, and priorities.*

**Contact:** [Name], Taia Tiniyara | [Email] | [Phone / WhatsApp]
