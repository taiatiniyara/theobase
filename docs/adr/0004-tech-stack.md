...
| Forms           | **Zod** (validation) + native React forms                     | Zod schemas shared between client and DO for end-to-end type safety on giving records and member forms. Form state managed via React's native controlled components, not an external form library — keeps the bundle smaller and avoids dependency lock-in. |
...
- tanStack Query's cache layer becomes the client-side source of truth, synced with IndexedDB and the DO via sync polling. This is complex to set up but eliminates the ad-hoc sync code that would otherwise grow organically.
