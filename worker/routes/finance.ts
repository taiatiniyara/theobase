...
export async function handleGetExpenseCategories(request: Request, env: Env): Promise<Response> {
  ...
  let query = "SELECT id, name, conference_id, created_at FROM expense_categories";
  const params: number[] = [];
...
export async function handleGetBatches(request: Request, env: Env): Promise<Response> {
  ...
  let query = `SELECT b.id, b.church_id, b.sabbath_date, b.status,
    b.confirmed_by_1, b.confirmed_at_1, b.confirmed_by_2, b.confirmed_at_2, b.created_at,
...
export async function handleCreateBatch(request: Request, env: Env): Promise<Response> {
  ...
  const result = await env.DB.prepare(
    `INSERT INTO offering_batches (church_id, sabbath_date) VALUES (?, ?) RETURNING id`
  )
    .bind(body.churchId, body.sabbathDate)
    .first<{ id: number }>();
...
export async function handleCreateTransaction(request: Request, env: Env): Promise<Response> {
  ...
  const txnUuid = uuid();
  const result = await env.DB.prepare(
    `INSERT INTO transactions (church_id, fund_id, type, amount, description, batch_id, created_by, uuid)
