...
- **Magic Link** — email-based passwordless authentication. Worker generates a signed RS256 JWT (10-min expiry, single-use), Cloudflare Email Routing delivers the login link. Session JWT (7-day, refreshed transparently when >24h remain) stored as both httpOnly cookie (server-side) and non-httpOnly cookie (client-side for offline header attachment). Role assignment via invite: clerk sends role-specific invite links. See `docs/adr/0006-authentication.md`.
- **JWT Payload** — carries `userId`, `churchId`, `role` (clerk / treasurer / counter / pastor / department-head / board-member / member / interest / visitor / conference-treasurer / conference-secretary / conference-president / auditor / operator), `tokenVersion`. RS256 signed. Worker middleware validates on every request. DO enforces role on every mutation — defence in depth. On role change, token version increments, invalidating all existing JWTs for that user. Session JWTs refresh transparently when >24h remain.
...
- **MFA (TOTP)** — optional time-based one-time passcode as a second authentication factor. Required for treasurer and counter roles (they handle money). Clerk and pastor may enable voluntarily.
...
- **Demo Seed** — `pnpm seed:demo` provisions "Suva Central SDA Church" under Fiji Mission with 120 synthetic members, 6 months of giving records, 24 committed batches, 2 active demo batches (one pending counter2, one disputed), and 6 demo user accounts. See `docs/adr/0013-demo-seed.md`.
- **Conference Onboarding** — self-serve: Conference admin signs up at `theobase.app`, enters billing details, lands in the Suva Central demo church with a guided product tour. Activates real churches via sidebar. No sales call required.
...
- **Restore Drill** — a Worker Cron trigger runs monthly: picks a random church, replays its event log into a fresh DO, verifies state hash matches. Failure → P1 alert. See `docs/adr/0016-restore-drill.md`.
...
17. Build secure, certify later — SOC 2 and other compliance deferred until a Conference requires it. Architecture is SOC2-ready: event log immutability, PII isolation, MFA for financial roles, data classification and retention policies, privacy rights workflow. See `docs/adr/0010-soc2-readiness.md`.
...
21. Observability built-in — v1 ships the data pipeline (PWA client wrapper, DO error catches, D1 errors + sync_health tables, R2 raw payloads). The observability UI arrives in v1.5. See `docs/adr/0014-observability.md`.
