...
describe("conference dashboard", () => {
  ...
  beforeAll(async () => {
    ...
    const loginSr = await SELF.fetch("http://localhost/api/auth/login", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ email: "cd@t.com", password: "password123" }),
    });
    ...
    await env.DB.prepare("UPDATE users SET email_verified = 1 WHERE email = ?")
      .bind("pastor@t.com")
      .run();
    const loginPr = await SELF.fetch("http://localhost/api/auth/login", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ email: "pastor@t.com", password: "password123" }),
    });
    const pb = (await loginPr.json()) as { accessToken: string };
