...
export function registerOrganizationRoutes(app: AppType) {
  app.post("/organizations", requireAuth(), async (c) => {
    ...
    const [existing] = await db
      ...
      .where(
        and(
          eq(schema.organization.name, parsed.data.name),
          eq(schema.organization.type, parsed.data.type)
        )
      );
    if (existing) {
      return c.json(
        {
          error: `A ${parsed.data.type.replace(/_/g, " ")} named "${parsed.data.name}" already exists.`,
        },
        409
      );
    }
    ...
    await db
      ...
      .values({ id, ...parsed.data, createdAt: now });
    const [r] = await db
      .select()
      .from(schema.organization)
  ...
  app.get("/organizations", requireAuth(), async (c) => {
    const type = c.req.query("type") as
      | (typeof schema.organization.type.enumValues)[number]
      | undefined;
    const conditions = type ? [eq(schema.organization.type, type)] : [];
  ...
  app.get("/organizations/:id", requireAuth(), async (c) => {
    const [r] = await getDb(c)
      .select()
      .from(schema.organization)
      .where(eq(schema.organization.id, c.req.param("id")));
  ...
  app.get("/departments", requireAuth(), loadRoles(), async (c) => {
    ...
    const rows = await getDb(c)
      ...
      .where(eq(schema.department.congregationId, congregationId))
  ...
  app.patch(
    ...
    async (c) => {
      ...
      await db
        ...
        .set(parsed.data)
        .where(eq(schema.department.id, c.req.param("id")));
      const [r] = await db
        .select()
        .from(schema.department)
        .where(eq(schema.department.id, c.req.param("id")));
