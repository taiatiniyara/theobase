...
export function registerBoardroomRoutes(app: AppType) {
  ...
  app.post(
    ...
    requireAuth(),
    loadRoles(),
    requireWriteAccess("clerk", "treasurer", "elder"),
    async (c) => {
      const db = getDb(c);
      ...
      const [created] = await db
        ...
        .where(eq(schema.boardMeeting.id, id));

      const doStub = getCongregationDO(c, congregationId);
      if (doStub)
        await doStub.meetingUpdated({
          id,
          status: "draft",
          date: parsed.data.date,
        });
      ...
      return c.json(
        {
          ...created,
          agenda: created.agenda ? JSON.parse(created.agenda) : null,
        },
        201
      );
    }
  ...
  app.get(
    ...
    requireAuth(),
    loadRoles(),
    requireRole("clerk", "treasurer", "elder"),
    async (c) => {
      const db = getDb(c);
      const congregationId = c.get("congregationId");
  ...
  app.get(
    ...
    async (c) => {
      ...
      const [meeting] = await db
        ...
        .where(
          and(
            eq(schema.boardMeeting.id, c.req.param("id")),
            eq(schema.boardMeeting.congregationId, congregationId)
          )
        );
  ...
  app.post(
    ...
    async (c) => {
      ...
      const nextNumber =
        decisions.length > 0
          ? Math.max(...decisions.map((d: any) => d.number)) + 1
          : 1;
      ...
      const [created] = await db
        .select()
        .from(schema.boardDecision)
        .where(eq(schema.boardDecision.id, id));
      ...
      if (congregationId) {
        const doStub = getCongregationDO(c, congregationId);
        if (doStub)
          await doStub.decisionRecorded(meetingId, {
            id,
            title: parsed.data.title,
            voteOutcome: parsed.data.voteOutcome || "pending",
          });
      }
  ...
  app.get(
    ...
    async (c) => {
      ...
      const congregationId = c.get("congregationId")!;
      const [meeting] = await db
        .select({ id: schema.boardMeeting.id })
        .from(schema.boardMeeting)
        .where(
          and(
            eq(schema.boardMeeting.id, c.req.param("id")),
      ...
      const rows = await db
        ...
        .orderBy(asc(schema.boardMinute.createdAt));
      return c.json(rows);
    }
  );
  ...
  app.patch(
    ...
    async (c) => {
      ...
      const [current] = await db
        .select()
        .from(schema.boardMinute)
        .where(eq(schema.boardMinute.id, minuteId));
      ...
      const base =
        parsed.data.baseVersion !== undefined
          ? { version: parsed.data.baseVersion, content: current.content }
          : { version: current.revisionNumber, content: current.content };
      const local = {
        version: current.revisionNumber + 1,
        content: parsed.data.content,
        nodeId,
      };
      const remote = {
      ...
      if (fork) {
        ...
        const conflict = await db
          .select()
          .from(schema.boardMinute)
          .where(eq(schema.boardMinute.meetingId, current.meetingId));
        return c.json(
          {
            fork: true,
            conflict: true,
            message:
              "Revision conflict detected. A conflict version has been saved for manual resolution.",
            versions: conflict,
          },
          409
        );
